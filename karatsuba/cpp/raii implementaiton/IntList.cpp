...
// TODO: So this is where we're testing the very existence of the IntList. So, we test the invariants of
// the IntList: basically, it always has a valid value. An initialized vector containing unsigned, 
// zero-trimmed (when constructor-specified), single-digit values.
//
// ALSO maybe, does it properly disappear when it goes out of scope! How would you do that? Since that kind
// of test is done by basically letting to to references for it? Can you query to see if some chunk of memory
// has been let go?
//

// TODO: keep in mind we're also trying to come up with a test-based design that flags problems
// as soon as what it's working with downstream finds problems. Finding problems early... is there
// something we can do with closures, a bubble of "origin" that pops when a problem actually surfaces?
// Maybe when a problem happens, the local fail-partner can then do some interrogation with the base
// system to get at what actually happened, and maybe get some good advice for what THEY did wrong, if
// indeed they did something wrong? Basically what was expected didn't meet what was provided?

// // TODO: but we can't really do this here! we have to wait until we get the equality checking
// // stuff implemented and then this belongs under THAT. All we can do with this sucker is slap it
// // inside of a try-block and make sure it comes and goes without blowing up.
// //
// {   //
//     // IntList::IntList unit testing
//     //
//     unsigned int n = 123;
//     auto il=IntList(n); 
//     std::assert(il == 123);
//     std::assert(il == {1,2,3}); // TODO: do we have to do initialization_list stuff to make this happen?
// }



// TODO: card this?
// TODO: so how should we be handling test-driven dev at this point?  When we plop a class declaration down, then
// I guess we should maybe then do some tests that check it? And then whenever a member appears, do the same? Kind of
// a matched-pair to a functional style: we call the functions to say what we want done, we have test cases to adversarially
// check and make sure that the function does what it's supposed to do. Almost a sort of "cryptographic" protocol in a sense
// of being un-trusty.
//
// So in sticking with this philosophy, maybe our testing should be at the function call level; never something to check that
// some internal state is like this or that... our constructors for ex: DO NOT have some invasive check to make sure the 
// unsigned int we pass to the constructor is "correctly stored" inside. but DO have a full-check of initialization followed
// by read-out via whatever output methodology we have. 
//
// Maybe what we mean to say is that every block should have it's own functional-style validation checks, adversarially testing
// out all of the stuff it calls; not really trusting it. But only in test mode, natch.
